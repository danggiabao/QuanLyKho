...
END
GO 
CREATE PROCEDURE ThemPX(@MAPHIEU VARCHAR(8),@NGAYXUAT DATE,@MAKHO NVARCHAR(8))
AS
BEGIN
		INSERT INTO PHIEUXUAT
	VALUES(@MAPHIEU,@NGAYXUAT,@MAKHO)
END
GO
CREATE PROCEDURE SuaPX(@MAPHIEU VARCHAR(8),@NGAYXUAT DATE,@MAKHO NVARCHAR(8))
AS
BEGIN
	UPDATE PHIEUXUAT
	SET NGAYXUAT=@NGAYXUAT,MAKHO=@MAKHO
	WHERE MAPHIEU=@MAPHIEU
			
END
GO
CREATE PROCEDURE XoaPX(@MAPHIEU VARCHAR(8))
AS
BEGIN 
...
END		
GO
CREATE PROCEDURE ThemCTPX(@MAPHIEU VARCHAR(8),@MAVT VARCHAR(8),@DONVITINH NVARCHAR(15),@SOLUONG INT)
AS
BEGIN
		INSERT INTO CHITIETPHIEUXUAT
	VALUES(@MAPHIEU,@MAVT,@DONVITINH,@SOLUONG)
END
GO
CREATE PROCEDURE SuaCTPX(@MAPHIEU VARCHAR(8),@MAVT VARCHAR(8),@DONVITINH NVARCHAR(15),@SOLUONG INT)
AS
BEGIN
	UPDATE CHITIETPHIEUXUAT
	SET MAVT=@MAVT,DONVITINH=@DONVITINH,SOLUONG=@SOLUONG
	WHERE MAPHIEU=@MAPHIEU
			
END
GO
CREATE PROCEDURE XoaCTPX(@MAPHIEU VARCHAR(8))
AS
BEGIN 
